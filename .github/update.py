cat > /mnt/user-data/outputs/update.py << 'EOF'
import requests, os, json, re, datetime

API_KEY = os.environ.get("NEWS_API_KEY", "")

CATEGORIES = [
    {"cat": "news",          "q": "España OR Latino OR internacional",   "label": "新闻",  "color": "#2980B9"},
    {"cat": "science",       "q": "ciencia OR tecnología OR salud",       "label": "科普",  "color": "#16A085"},
    {"cat": "football",      "q": "fútbol OR Champions OR LaLiga",        "label": "足球",  "color": "#27AE60"},
    {"cat": "cinema",        "q": "cine OR película OR festival OR serie","label": "电影",  "color": "#8E44AD"},
    {"cat": "entertainment", "q": "música OR cultura OR arte",            "label": "娱乐",  "color": "#D35400"},
]

SOURCES = "el-mundo,bbc-news"

TRANSLATIONS = {
    "España": "西班牙", "gobierno": "政府", "presidente": "总统/主席",
    "ministro": "部长", "partido": "政党/比赛", "elecciones": "选举",
    "economía": "经济", "crisis": "危机", "acuerdo": "协议",
    "fútbol": "足球", "gol": "进球", "campeón": "冠军",
    "película": "电影", "estreno": "首映", "director": "导演",
    "ciencia": "科学", "investigación": "研究", "descubrimiento": "发现",
    "salud": "健康", "tecnología": "技术", "inteligencia": "智能",
    "cultura": "文化", "música": "音乐", "arte": "艺术",
    "mundo": "世界", "país": "国家", "ciudad": "城市",
    "años": "年", "millones": "百万", "personas": "人们",
    "nuevo": "新的", "gran": "伟大的", "primera": "第一",
}

def fetch_articles(q, n=2):
    url = "https://newsapi.org/v2/everything"
    params = {
        "q": q, "language": "es", "pageSize": n,
...
def simple_translate(text):
    ...
    for es, zh in TRANSLATIONS.items():
        result = re.sub(r'\b' + es + r'\b', zh, result, flags=re.IGNORECASE)
    return result

def make_sentences(text, source_url):
    raw = re.split(r'(?<=[.!?])\s+', (text or "").strip())
    sentences = [s.strip() for s in raw if len(s.strip()) > 30][:5]
    if not sentences:
        sentences = [text[:200]] if text else ["Sin contenido disponible."]
    result = []
    for i, s in enumerate(sentences):
        result.append({
            "id": f"s{i}",
            "es": s,
            "zh": simple_translate(s)
        })
    return result

def make_pattern(title):
    patterns = [
        {
            "sent": "El gobierno anunció nuevas medidas para afrontar la situación",
            "analysis": "【anunciar + 名词从句/名词】= 宣布……，是新闻语体最常用动词之一。afrontar（应对，面对）比 enfrentar 更书面化，常见于政治经济类报道。para + infinitivo 表示目的。",
            "formula": "anunciar + 名词 = 宣布……"
        },
        {
            "sent": "Según los expertos, la situación podría mejorar en los próximos meses",
            "analysis": "【según + 名词】= 根据……，据……，是引用消息来源的标准新闻句型。podría + infinitivo 是条件式（Condicional Simple），表示可能性推测，比直接陈述语气更谨慎。",
            "formula": "según + 消息来源 + podría + inf. = 据……，可能会……"
        },
        {
            "sent": "La cifra supera con creces las expectativas iniciales",
            "analysis": "【superar con creces】= 大幅超过，远超，是B2-C1新闻语体的固定短语。con creces（字面意思：带着增量）作副词短语，强调程度之深。expectativas（预期）是新闻和商业语体的高频词。",
            "formula": "superar con creces = 远超，大幅超过"
        },
    ]
    import hashlib
    idx = int(hashlib.md5(title.encode()).hexdigest(), 16) % len(patterns)
    return [patterns[idx]]
...
def build_articles():
    ...
    for cat in CATEGORIES:
        fetched = fetch_articles(cat["q"], 2)
        for a in fetched[:2]:
            ...
            if not title or title == "[Removed]":
                continue
            sentences = make_sentences(desc, url)
            if not sentences:
                continue
            articles.append({
                "id": art_id,
                ...
                "url": url,
                "title": title,
                "subtitle": simple_translate(title),
                "content": sentences,
                "patterns": make_pattern(title),
            })
            art_id += 1
...
.tab{{padding:13px 15px;font-size:13px;font-weight:800;color:var(--muted);cursor:pointer;border:none;background:none;border-bottom:3px solid transparent;margin-bottom:-2px;transition:all .2s;font-family:inherit}}
...
function toggleChinese(){{
  ...
  var btn=document.getElementById('zhToggle');if(btn){{btn.textContent=showChinese?'隐藏中文翻译':'显示中文翻译';btn.className=showChinese?'ctrl on':'ctrl';}}
}}
function showWordPopup(word,sentence){{
  ...
  body.innerHTML='<div class="pw">'+word+'</div><div class="pty">西班牙语词汇</div><div class="pm" style="font-size:15px;color:var(--muted)">词典收录中...</div>'+ctxHtml+'<div class="pacts"><button class="addbtn" id="addWordBtn">加入词库</button><button class="clsbtn" id="closeBtn">关闭</button></div>';
